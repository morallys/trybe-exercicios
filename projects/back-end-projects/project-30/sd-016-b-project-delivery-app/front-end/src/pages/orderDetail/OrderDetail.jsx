        ...
        {order.map((orderInfo) => (
          <div className="flex justify-between" key={ orderInfo.id }>
            ...
            <div>
              ...
              >
                <span id="test">{orderInfo.seller.name}</span>
              </label>
            </div>
            <strong
              data-testid={ 'customer_order_details'
            ...
            <button
              className="bg-green-500 disabled:bg-black"
              disabled={ orderInfo.status === 'Pendente' }
              data-testid={ 'customer_order_details'
              + '__button-delivery-check' }
              type="button"
            >
            ...
            <div key={ i } className="flex flex-col">
              {products.map((p, ind) => (
                <div className="flex gap-2" key={ p.id }>
                  <span
                    data-testid={ 'customer_order_details'
                    + `__element-order-table-item-number-${ind}` }
                  >
                    {ind + 1}
                  </span>
                  <span
                    data-testid={ 'customer_order_details'
                    + `__element-order-table-name-${ind}` }
                  >
                    {p.name}
                  </span>
                  <span
                    data-testid={ 'customer_order_details'
                    + `__element-order-table-sub-total-${ind}` }
                  >
                    {p.price}
                  </span>
                  <span
                    data-testid={ 'customer_order_details'
                    + `__element-order-table-quantity-${ind}` }
                  >
                    {p.SaleProduct.quantity}
                  </span>
                  <span
                    data-testid={ 'customer_order_details'
                    + `__element-order-total-price-${ind}` }
                  >
                    {(Number(p.price) * Number(p.SaleProduct.quantity)).toFixed(
                      2,
                    )}
                  </span>
                </div>
