...
SELECT MAX(SALARY) - MIN(SALARY) AS 'Diferença salário Max/Min' FROM employees;
...
SELECT SUM(SALARY) AS total_value_salary_employees FROM employees;

-- 5. Escreva uma query que exiba quatro informações: o maior salário, o menor salário, a soma de todos os salários e a média dos salários. Todos os valores devem ser formatados para ter apenas duas casas decimais.

SELECT
  FORMAT(MAX(SALARY), 2) AS 'HIGHER SALARY',
  FORMAT(MIN(SALARY), 2) AS 'LOWER SALARY',
  FORMAT(SUM(SALARY), 2) AS 'SUM ALL SALARY',
  FORMAT(AVG(SALARY), 2) AS 'AVERAGE ALL SALARY'
FROM employees;

-- 6. Escreva uma query que exiba a quantidade de pessoas que trabalham como pessoas programadoras ( IT_PROG ).

SELECT JOB_ID, COUNT(JOB_ID) AS job
  FROM employees
  GROUP BY JOB_ID
...
SELECT DEPARTMENT_ID, COUNT(DEPARTMENT_ID) AS number_employees, AVG(SALARY)
  FROM employees
